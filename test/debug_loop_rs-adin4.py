...
#//=========================================================================================================================
def FixUp_RSModule_Data(read_data, debug=False):
...
def RS_Read (ReadModule, RS_Code, numChannels, debug):
	...
	while count_attemps_read < max_attemps_read:
		...
		if lenght_readData > 0:	
			...
			if check_crc == 0:
				crc_flag = True
				if debug: print "CRC check BEFORE Fixed up the data received"

			read_data_fixedup = FixUp_RSModule_Data(read_data, debug)		# Function that fix all modbus data to real data

			if crc_flag == False:
				check_crc = checking_CRC_sum(read_data_fixedup)					# Checking if the CRC of data is OK
				if debug: print "CRC check AFTER Fixed up the data received"

			if debug: print "Check CRC: " + str(hex(check_crc))

			flag_pasted_frames = False		# Flag of joined frames. flag=True: 
			# poner un if len(read_data_fixedup) > 5:
			if ord(read_data_fixedup[1]) == RS_Code and check_crc == 0 and ord(read_data_fixedup[2]) == module_number:
				# Extraer, procesar y retornar arreglo
				if debug: print "No spliting, CRC ===== OK == OK == OK ====="
				count_attemps_read = 3
				pass
			else:
				# read_data_fixedup = read_data_fixedup[0:read_data_fixedup.index(chr(0x03))+1]
				if debug: print "Looking two frames together"
				try:
					read_data_fixedup.index(chr(0x03)+chr(0x02))	# Verify if there are two joined frames searching 0x03 and 0x02 in hex
				except Exception as e:
					...
					read_data1 = read_data_fixedup[0 : read_data_fixedup.index(chr(0x03)+chr(0x02)) + 1]
					read_data2 = read_data_fixedup[read_data_fixedup.index(chr(0x03)+chr(0x02)) + 1 : lenght_readData + 1]
					...
					if ord(read_data1[1]) == RS_Code and check_crc1 == 0 and ord(read_data_fixedup[2]) == module_number:
						# Extraer, procesar y retornar arreglo de la parte 1
						read_data = read_data1
						count_attemps_read = max_attemps_read
						pass
					elif ord(read_data2[1]) == RS_Code and check_crc2 == 0 and ord(read_data_fixedup[2]) == module_number:
			...
			# if read_data_fixedup.index(chr(0x03)+chr(0x02)):
			# 	read_data_fixedup = read_data_fixedup[read_data_fixedup.index(chr(0x03)+chr(0x02))+1:lenght_readData+1]
			
			if not flag_pasted_frames:
				if debug: print "Real size: " + str(read_data_fixedup.index(chr(0x03))+1)
				RawData = []
				if len(read_data_fixedup) > 6:
					# RS_ADIN4 verify the check sum CRC before the proccess the data
					# if RS_Code == RS_ADIN4_Code: read_data_fixedup = FixUp_RSModule_Data(read_data_fixedup)	# Function that fix all modbus data to real data***************
					k = 3
					data = 0
					if debug: print "Array size: " + str(len(read_data_fixedup))
					"""Cicle to extract the real ADC data to proccess in the specific application"""
					for n in range(0, numChannels):
						data = ( ord(read_data_fixedup[k]) * 256 ) + ord(read_data_fixedup[k+1])
						if data >= 11000:
							data = -1
			...
			time.sleep(.03)
		# if there is no data in read_data_fixedup fill the array with -4 to show an error.
		else:
			RawData = []
			for n in range(0, numChannels):
				RawData.insert(n, -4)
...
if __name__ == '__main__':
	...
	while True:
		if OpenSerialfor_Moacon() is True:
			
			for ModuleNumber in range(1,4):
				results = RS_ADIN4(ModuleNumber, True)
				print "Module " + str(ModuleNumber) + ": "
